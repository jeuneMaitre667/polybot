...
function parseForbidWindowFromEnv() {
  ...
  const rl = process.env.ENTRY_FORBIDDEN_LAST_SEC; // Changé de MIN à SEC pour la précision demandée (20s)
  
  let f_min = rf === undefined || rf === '' ? 0 : Number(rf);
  let l_sec = rl === undefined || rl === '' ? 0 : Number(rl);
  
  if (!Number.isFinite(f_min)) f_min = 4; // Défaut user
  if (!Number.isFinite(l_sec)) l_sec = 20; // Défaut user
  
  return {
    forbidFirstSec: f_min * 60,
    forbidLastSec: l_sec,
    entryForbidFirstMin: f_min,
    entryForbidLastSec: l_sec,
  };
}

const { forbidFirstSec: FORBID_FIRST_SEC, forbidLastSec: FORBID_LAST_SEC, entryForbidFirstMin, entryForbidLastSec } =
...
export const ENTRY_FORBID_LAST_MIN_RESOLVED = entryForbidLastSec;
