...
describe('liquidityUsdFromAsks', () => {
  ...
  it('somme prix×taille pour objets { price, size } dans la bande', () => {
    const asks = [
      { price: '0.775', size: '100' },
      { price: '0.97', size: '10' },
      { price: '0.975', size: '20' },
      { price: '0.98', size: '50' },
    ];
    // Bande défaut 77–78¢ : inclut 0,775 → 0.775*100 = 77.5
    expect(liquidityUsdFromAsks(asks)).toBe(77.5);
  });
  ...
  it('accepte tuples [price, size]', () => {
    ...
    // Bande défaut 77–78¢ : rien dans la bande
  ...
  it('expose les constantes de bande signal', () => {
    expect(ORDER_BOOK_SIGNAL_MIN_P).toBe(0.77);
    expect(ORDER_BOOK_SIGNAL_MAX_P).toBe(0.78);
  });
});
