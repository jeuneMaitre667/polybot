    ...
    for (const asset of SUPPORTED_ASSETS) {
      ...
      if (hasEvent && slug) {
        ...
        if (!state.currentSlotStrike || state.currentSlotStrike.slotSlug !== slug) {
           ...
           // v2026 : Initialisation du tracker de slot pour fixer le Strike
           state.currentSlotStrike = {
             slotSlug: slug,
             strike: null,
             isOfficial: false,
             startMs: Date.now()
           ...
           state.pythRefAtMs = Date.now();
           console.log(`[${asset}] 📊 Slot tracking updated for ${slug}. Momentum anchor set to ${curPyth}.`);
        }

        // v2026 : Capture du Strike officiel depuis les signaux Polymarket
        if (!state.currentSlotStrike.isOfficial && signals.length > 0) {
           const firstSignalWithStrike = signals.find(s => s.strike != null && Number(s.strike) > 0);
           if (firstSignalWithStrike) {
              state.currentSlotStrike.strike = Number(firstSignalWithStrike.strike);
              state.currentSlotStrike.isOfficial = true;
              console.log(`[${asset}] ✅ Official Strike fixed at ${state.currentSlotStrike.strike} for ${slug}`);
           }
        }
